...
if(year >= 2014){
  ...
  for(i in 1:nrow(reg.season)){
    ...
    # Assigns points according to 1-0 point system: 1 point for a win, 0 for a loss
...
if(year > 2010 & year < 2014){
  ...
  for(i in 1:nrow(reg.season)){
    ...
    # Assigns points: 1 for a win, 0 for a loss (any kind)
    reg.season[["Winner Points Awarded"]][i] <- 1
    reg.season[["Loser Points Awarded"]][i] <- 0
  }
...
if(year >= 2000 & year < 2005){
  ...
  for(i in 1:nrow(reg.season)){
    ...
    # Assigns points: 1 for a win, 0 for a loss, .5 for a tie
    if(reg.season$`OT/SO`[i] == ""){
      ...
      if(reg.season$`OT/SO`[i] != "" & (reg.season$`Goals Away`[i] != reg.season$`Goals Home`[i])){
        ...
        reg.season[["Winner Points Awarded"]][i] <- .5
        reg.season[["Loser Points Awarded"]][i] <- .5
      }
    }
  }
